//! Self-contained constant-time SHA-256, HMAC-SHA256, and PBKDF2-HMAC-SHA256.
...
impl Sha256 {
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        ...
        if idx >= 56 {
            for b in &mut self.buffer[idx + 1..64] {
                *b = 0;
            }
            self.process_block();
            self.buffer = [0u8; 64];
        } else {
            for b in &mut self.buffer[idx + 1..56] {
                *b = 0;
            }
        }
...
pub fn pbkdf2_hmac_sha256(password: &[u8], salt: &[u8], iterations: u32, out: &mut [u8]) {
    ...
    while offset < out.len() {
        ...
        for _ in 1..iterations {
            let mut next_hmac = HmacSha256::new(password);
            next_hmac.update(&u);
            u = next_hmac.finalize();
            for j in 0..32 {
                t[j] ^= u[j];
            }
        }
